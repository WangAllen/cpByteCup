...
fExtractFeatures <- function(x) {
  ...
  asked_q <- filter(question_info, qid %in% unique(asked_qid))
  ...
  answered_q <- filter(question_info, qid %in% unique(answered_qid))
  ...
  nonAnswered_q <- filter(question_info, qid %in% unique(nonAnswered_qid))
  ...
  wordIDs_diff <- setdiff(wordIDs_nonanswered_total, wordIDs_answered_total)
  f_wordID_total_diff <- length(intersect(wordIDs_q$wordID, wordIDs_diff$wordID)) / length(union(wordIDs_q$wordID, wordIDs_diff$wordID))
  
  # feature 24: f_wordID_same_diff
  # wordIDs_q and setdiff(wordIDs_nonanswered_same, wordIDs_answered_same)
  wordIDs_diff <- setdiff(wordIDs_nonanswered_same, wordIDs_answered_same)
  f_wordID_same_diff <- length(intersect(wordIDs_q$wordID, wordIDs_diff$wordID)) / length(union(wordIDs_q$wordID, wordIDs_diff$wordID))
  ...
  charIDs_diff <- setdiff(charIDs_nonanswered_total, charIDs_answered_total)
  f_charID_total_diff <- length(intersect(charIDs_q$charID, charIDs_diff$charID)) / length(union(charIDs_q$charID, charIDs_diff$charID))
  
  # feature 29: f_charID_same_diff
  # charIDs_q and setdiff(charIDs_nonanswered_same, charIDs_answered_same)
  charIDs_diff <- setdiff(charIDs_nonanswered_same, charIDs_answered_same)
  f_charID_same_diff <- length(intersect(charIDs_q$charID, charIDs_diff$charID)) / length(union(charIDs_q$charID, charIDs_diff$charID))
